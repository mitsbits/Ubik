...
{
    public class UserStore : UserStoreBase<UbikUser, UbikRole, AuthDbContext>, IUserStoreWithCustomClaims<int>, IUbikUserStore<int>
    {
        public UserStore(
            IDbContextScopeFactory dbContextScopeFactory, IUserRepository _userRepository, IRoleRepository roleRepository,
            IdentityErrorDescriber describer = null)
            : base(dbContextScopeFactory, _userRepository, roleRepository, describer)
        { }

        public async Task<IEnumerable<string>> GetRoleNamesForUser(int userId, CancellationToken cancellationToken = default(CancellationToken))
        {
            using (_dbContextScopeFactory.CreateReadOnly())
            {
                return await _userRepo.GetRoleNames(userId, cancellationToken);
            }
        }
        ...
        public async Task<IEnumerable<Claim>> RoleRelatedClaims(int userId)
        {
            using (_dbContextScopeFactory.CreateReadOnly())
            {
                return (await _userRepo.RoleRelatedClaims(userId, default(CancellationToken))).Select(x => new Claim(x.ClaimType, x.ClaimValue));
            }
        }
    }

    public abstract class UserStoreBase<TUser, TRole, TContext> :
        ...
        where TContext : AuthDbContext
    {
        ...
        public UserStoreBase(
            IDbContextScopeFactory dbContextScopeFactory, IUserRepository userRepository, IRoleRepository roleRepository, IdentityErrorDescriber describer = null)
        {
        ...
        public virtual async Task<TUser> FindByIdAsync(string userId, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (_dbContextScopeFactory.CreateReadOnly())
            {
                return await _userRepo.GetAsync(x => x.Id == id, x => x.Claims, x => x.Roles, x => x.Logins) as TUser;
            }
        }
        ...
        public virtual int ConvertIdFromString(string id)
        {
            ...
            return Convert.ToInt32(id); //(int)TypeDescriptor.GetConverter(typeof(int)).ConvertFromInvariantString(id);
        }
        ...
        public virtual async Task<TUser> FindByNameAsync(string normalizedUserName, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (_dbContextScopeFactory.CreateReadOnly())
            {
                return (await _userRepo.GetAsync(u => u.NormalizedUserName == normalizedUserName, u => u.Claims, u => u.Roles, u => u.Logins)) as TUser;
            }
        }
        ...
        public async virtual Task AddToRoleAsync(TUser user, string roleName, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (var db = _dbContextScopeFactory.CreateWithTransaction(IsolationLevel.ReadCommitted))
            {
                var roleEntity = await _roleRepo.GetAsync(x => x.Name.ToUpper() == roleName.ToUpper());
                if (!user.Roles.Any(x => x.RoleId == roleEntity.Id))
                {
                    await _userRepo.SetUserRole(user.Id, roleEntity.Id);
                    user.Roles.Add(new UbikUserRole() { RoleId = roleEntity.Id, UserId = user.Id });
                }
        ...
        public async virtual Task RemoveFromRoleAsync(TUser user, string roleName, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (var db = _dbContextScopeFactory.CreateWithTransaction(IsolationLevel.ReadCommitted))
            {
                ...
                if (roleEntity != null)
                {
                    user = await _userRepo.GetAsync(x => x.Id == user.Id, x => x.Roles) as TUser;
                    var roleToRemove = user.Roles.FirstOrDefault(x => x.RoleId == roleEntity.Id);
        ...
        public virtual async Task<IList<string>> GetRolesAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            {
                var userId = user.Id;
                return (await _userRepo.GetRoleNames(userId, cancellationToken)).ToList();
            }
        }
        ...
        public virtual async Task<bool> IsInRoleAsync(TUser user, string roleName, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (_dbContextScopeFactory.CreateReadOnly())
            {
                ...
                var role = await _roleRepo.GetAsync(r => userroles.Contains(r.Id) && r.Name == roleName);
                return role != null;
            }
        }
        ...
        public async virtual Task<IList<Claim>> GetClaimsAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (_dbContextScopeFactory.CreateReadOnly())
            {
                var userWithClaims = await _userRepo.GetAsync(x => x.Id == user.Id, x => x.Claims);
                return userWithClaims.Claims.Select(x => new Claim(x.ClaimType, x.ClaimValue)).ToList();
            }
        }
        ...
        public virtual async Task AddClaimsAsync(TUser user, IEnumerable<Claim> claims, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (var db = _dbContextScopeFactory.CreateWithTransaction(IsolationLevel.ReadCommitted))
            {
                ...
                {
                    if (!userWithClaims.Claims.Any(x => x.ClaimType == claim.Type && x.ClaimValue == claim.Value))
                        userWithClaims.Claims.Add(new UbikUserClaim(claim.Type, claim.Value));
                }
                await db.SaveChangesAsync(cancellationToken);
            }
        }
        ...
        public async virtual Task ReplaceClaimAsync(TUser user, Claim claim, Claim newClaim, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (var db = _dbContextScopeFactory.CreateWithTransaction(IsolationLevel.ReadCommitted))
            {
                user = await _userRepo.GetAsync(x => x.Id == user.Id, x => x.Claims) as TUser;
                var matchedClaims = user.Claims.Where(x => x.ClaimType == claim.Type && x.ClaimValue == claim.Value);
        ...
        public async virtual Task RemoveClaimsAsync(TUser user, IEnumerable<Claim> claims, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            {
                user = await _userRepo.GetAsync(x => x.Id == user.Id, x => x.Claims) as TUser;
                var matchedClaims = new List<UbikUserClaim>();
                foreach (var matchedClaim in claims)
                {
                    var hit = user.Claims.FirstOrDefault(x => x.ClaimType == matchedClaim.Type && x.ClaimValue == matchedClaim.Value);
                    if (hit != null) matchedClaims.Add(hit);
                }
                foreach (var hit in matchedClaims)
                {
                    user.Claims.Remove(hit);
                }
                await _userRepo.UpdateAsync(user);
                await db.SaveChangesAsync(cancellationToken);
            }
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (var db = _dbContextScopeFactory.CreateWithTransaction(IsolationLevel.ReadCommitted))
            {
                user = await _userRepo.GetAsync(x => x.Id == user.Id, x => x.Logins) as TUser;
                user.Logins.Add(l);
                await db.SaveChangesAsync(cancellationToken);
            }
        }
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (var db = _dbContextScopeFactory.CreateWithTransaction(IsolationLevel.ReadCommitted))
            {
                await _userRepo.RemoveLogin(userId, loginProvider, providerKey, cancellationToken);
                await db.SaveChangesAsync(cancellationToken);
            }
        }
        ...
        public async virtual Task<IList<UserLoginInfo>> GetLoginsAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            {
                user = await _userRepo.GetAsync(x => x.Id == userId, x => x.Logins) as TUser;
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (var db = _dbContextScopeFactory.CreateReadOnly())
            {
                Expression<Func<UbikUser, bool>> predicate = x => x.Logins.Any(l => l.LoginProvider == loginProvider && l.ProviderKey == providerKey);
                return await _userRepo.GetAsync(predicate) as TUser;
            }
        }
        ...
        public async virtual Task<TUser> FindByEmailAsync(string normalizedEmail, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (var db = _dbContextScopeFactory.CreateReadOnly())
            {
                Expression<Func<UbikUser, bool>> predicate = x => x.NormalizedEmail == normalizedEmail;
                return await _userRepo.GetAsync(predicate) as TUser;
            }
        }
        ...
        public async virtual Task<IList<TUser>> GetUsersForClaimAsync(Claim claim, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (var db = _dbContextScopeFactory.CreateReadOnly())
            {
                Expression<Func<UbikUser, bool>> predicate = x => x.Claims.Any(c => c.ClaimType == claim.Type && c.ClaimValue == claim.Value);
                return (await _userRepo.FindAllAsync(predicate, new[] { OrderByInfo<UbikUser>.SortAscending<UbikUser>(x => x.UserName) }))
                    .Cast<TUser>().ToList();
            }
        }
        ...
        public async virtual Task<IList<TUser>> GetUsersInRoleAsync(string roleName, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (var db = _dbContextScopeFactory.CreateReadOnly())
            {
                return (await _userRepo.GetUsersInRole(roleName, cancellationToken)).Cast<TUser>().ToList();
            }
        }
    }
}